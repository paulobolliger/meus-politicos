...
const SUGESTOES = [
  'O que propõe este projeto?',
  'Como isso afeta meu bolso?',
  'Qual é a situação atual da tramitação?',
]

export function IAPerguntaProjeto({ projetoId, tituloProjeto }: IAPerguntaProjetoProps) {
  ...
  const handleEnviar = async (texto: string) => {
    ...
    try {
      const response = await perguntarIA(projetoId, messages, texto)
  ...
  return (
    <div className="bg-[#121026] border border-[#7C3AED]/50 rounded-xl overflow-hidden shadow-[0_0_22px_rgba(124,58,237,0.18)] flex flex-col font-sans">
      ...
      <div className="bg-gradient-to-br from-[#2D1B69] via-[#1A113E] to-[#120A2B] border-b border-[#4C1D95]/40 p-3 flex items-start justify-between gap-1.5">
        <div className="flex items-start gap-1.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <span className="text-[9px] text-[#A78BFA] uppercase tracking-wider font-mono block truncate" title={`Dúvidas sobre o ${tituloProjeto}`}>
              Dúvidas: {tituloProjeto}
            </span>
          </div>
        </div>
      ...
      {/* Messages Feed */}
      <div className="p-3 space-y-3 max-h-[200px] overflow-y-auto custom-scrollbar flex-1 bg-[#090816]/70 min-h-[120px]">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-2 space-y-1.5">
            ...
            <p className="text-[11px] text-[#CBD5E1] max-w-xs leading-normal">
              Tem alguma dúvida sobre esta proposta? Faça uma pergunta abaixo ou use uma sugestão!
            </p>
          </div>
        ) : (
          messages.map((msg) => (
