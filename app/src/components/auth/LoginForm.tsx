...
export function LoginForm() {
  ...
  function signInUrl(connector?: string) {
    const base = `/api/auth/logto/sign-in?redirectTo=${encodeURIComponent(redirectTo)}`
  ...
  function entrarComOAuth(key: 'google' | 'x' | 'linkedin') {
    ...
    window.location.href = signInUrl(target)
  }
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants}>
        <h2 className="text-2xl font-bold text-slate-100 tracking-tight mb-1">Entrar</h2>
        <p className="text-xs text-slate-400">Identificação segura para o painel de monitoramento cívico.</p>
      </motion.div>
      ...
      <motion.div variants={itemVariants}>
        ...
        >
          {loadingEmail ? 'Redirecionando...' : 'Entrar com E-mail →'}
        </button>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="text-xs text-slate-400 mt-2 select-none">
        Não tem conta?{' '}
        <Link href="/cadastro" className="text-violet-400 hover:text-violet-300 font-semibold transition-colors duration-150">
          Cadastre-se
        </Link>
      </motion.div>
