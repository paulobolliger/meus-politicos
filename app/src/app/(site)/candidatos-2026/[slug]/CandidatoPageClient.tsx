      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <section style={{
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            <input
              type="text"
              placeholder="Pergunte algo sobre este candidato..."
              disabled
              style={{
                width: '100%', boxSizing: 'border-box',
                height: 38, padding: '0 44px 0 14px',
            ...
            <button disabled style={{
              ...
              fontSize: 16, color: 'var(--ink-3)', padding: 4,
            }}>
              ➤
            </button>
          </div>
          <p style={{ margin: '8px 0 0', fontSize: 10, color: 'var(--ink-3)', letterSpacing: '0.06em', fontFamily: 'var(--font-mono)' }}>
            DISPONÍVEL EM BREVE — INTELIGÊNCIA CÍVICA
          </p>
        </section>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ textAlign: 'center', paddingTop: 8 }}>
        <a href="#" style={{
          ...
          📄 Baixar PDF do Plano de Governo completo
        </a>
      </div>
    </div>
  )
...
function TabBens({ bens }: { bens: BemPatrimonial[] }) {
  const totalPorAno = new Map<number, number>()
  bens.forEach(b => totalPorAno.set(b.ano, (totalPorAno.get(b.ano) ?? 0) + b.valor))
  const anos = Array.from(totalPorAno.entries()).sort((a, b) => a[0] - b[0])
  const maxVal = Math.max(...anos.map(([, v]) => v), 1)
    ...
    return (
      <div style={{ textAlign: 'center', padding: '80px 20px' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--ink-3)', marginTop: 6, maxWidth: 380, margin: '8px auto 0', lineHeight: 1.6 }}>
          Os bens patrimoniais declarados ao TSE serão exibidos após a execução do ETL de candidatos 2026.
        </p>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ paddingBottom: 60 }}>
        ...
        {activeTab === 'bens'        && <TabBens       bens={bens} />}
      </div>
    </div>
  )
}
