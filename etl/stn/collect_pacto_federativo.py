...
def coletar_fpe(ano: int) -> dict[str, float]:
    """
    FPE via SICONFI — RREO Anexo 3 de cada estado.
    O endpoint antigo /ords/stn/tt/fpe foi descontinuado.
    Novo endpoint: https://apidatalake.tesouro.gov.br/ords/siconfi/tt/rreo
    Docs: https://apidatalake.tesouro.gov.br/docs/siconfi/

    O RREO-Anexo 03 contém as receitas de transferências constitucionais,
    incluindo o FPE (Fundo de Participação dos Estados) como rubrica de receita.
    Usamos o último bimestre (nr_periodo=6) do exercício.
    """
    # Códigos IBGE de 2 dígitos para cada estado
    COD_IBGE_UF = {
    ...
    for cod, sigla in COD_IBGE_UF.items():
        params = {
            'an_exercicio': ano,
            'in_periodicidade': 'B',   # Bimestral
            'nr_periodo': 6,            # 6º bimestre = acumulado do ano
            'co_tipo_demonstrativo': 'RREO',
            'no_anexo': 'RREO-Anexo 03',
            'co_poder': 'E',            # Executivo estadual
            'co_esfera': 'E',           # Estado
            'co_municipio_ibge': cod,   # código IBGE do estado (2 dígitos)
        }
        try:
            ...
            for item in items:
                # Filtrar rubrica FPE na coluna de conta/rubrica
                conta = str(item.get('no_conta', '') or item.get('co_conta', '')).upper()
                if 'FPE' in conta or 'PARTICIPAÇÃO DOS ESTADOS' in conta:
                    valor = float(item.get('vl_periodo', 0) or item.get('valor', 0) or 0)
                    fpe[sigla] = fpe.get(sigla, 0) + valor
        except Exception as exc:
            log.warning('Erro ao buscar RREO para %s (%s): %s', sigla, cod, exc)
        time.sleep(0.2)

    if fpe:
        log.info('FPE obtido via SICONFI RREO — %d estados', len(fpe))
        return {k: round(v / 1_000_000, 2) for k, v in fpe.items()}

    log.warning(
        'FPE via SICONFI retornou vazio. '
        'Verifique os parâmetros em: https://apidatalake.tesouro.gov.br/docs/siconfi/ '
        'Os dados do banco (seed da migration) permanecem intactos.'
    )
    return {}
