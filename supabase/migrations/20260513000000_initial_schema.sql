...
-- ============================================================
-- FEED EVENTOS
-- Eventos pré-computados para o feed do usuário
-- Cobre: trocas de partido, nomeações (DOU), notícias,
--        alertas de candidatura e outros eventos não tabelados
-- A VIEW feed_usuario une esta tabela + votacoes + gastos + discursos
-- ============================================================
...
CREATE TABLE feed_eventos (
  ...
  tipo            text not null,
  -- 'troca_partido' | 'nomeacao_cargo' | 'exoneracao_cargo'
  -- 'candidatura_registrada' | 'candidatura_deferida' | 'mandato_cassado'
  -- 'na_imprensa' | 'alerta_ficha_limpa' | 'afastamento'

  titulo          text not null,
  descricao       text,
  descricao_simples text,  -- versão traduzida/simplificada por IA
  ...
  -- Metadados flexíveis por tipo de evento
  metadata        jsonb,
  -- troca_partido:       {"partido_anterior": "PSL", "partido_novo": "PL"}
  -- nomeacao_cargo:      {"cargo": "Ministro da Fazenda", "orgao": "MF"}
  -- na_imprensa:         {"veiculo": "G1", "url": "https://...", "snippet": "..."}

  -- Lineage
  source_id          text,   -- 'dou' | 'camara' | 'g1' | 'agencia_brasil' etc.
  source_record_id   text,
  collected_at       timestamptz,
  link_fonte         text,
...
CREATE INDEX idx_feed_politico      ON feed_eventos(politico_id);
CREATE INDEX idx_feed_data          ON feed_eventos(data desc);
CREATE INDEX idx_feed_tipo          ON feed_eventos(tipo);
CREATE INDEX idx_feed_impacto       ON feed_eventos(impacto_nivel);
CREATE INDEX idx_feed_pol_data      ON feed_eventos(politico_id, data desc);
-- índice composto principal — query do feed: "últimos eventos de X"
CREATE INDEX idx_feed_source        ON feed_eventos(source_id);

-- RLS — leitura pública
ALTER TABLE feed_eventos ENABLE ROW LEVEL SECURITY;
CREATE POLICY "leitura publica" ON feed_eventos FOR SELECT USING (true);


-- ============================================================
-- VIEW: feed do usuário
-- Votações + gastos + discursos + feed_eventos dos políticos acompanhados
...
CREATE OR REPLACE VIEW feed_usuario AS
  ...
  SELECT
    ...
    p.id, p.nome, p.cargo, p.foto_url,
    pt.sigla        AS partido,
    fe.tipo         AS tipo_evento,
    fe.id           AS evento_id,
    fe.data,
    fe.descricao_simples AS descricao,
    fe.titulo       AS detalhe,
    t.slug AS tema, t.cor AS tema_cor, t.icone AS tema_icone,
    fe.impacto_nivel,
    fe.link_fonte
  FROM acompanhamentos a
  JOIN politicos    p  ON p.id = a.politico_id AND p.removido_em IS NULL
  JOIN feed_eventos fe ON fe.politico_id = p.id
  LEFT JOIN temas   t  ON t.id = fe.tema_id
  LEFT JOIN partidos pt ON pt.id = p.partido_id
